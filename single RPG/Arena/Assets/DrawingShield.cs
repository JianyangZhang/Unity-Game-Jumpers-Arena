            ...
            switch (shieldStatus)
            {
                case 1:
                    ...
                    if (sa.magnitude>10f)
                    {
                        ...
                        shieldGroup[pShieldGroup]=
                            Instantiate(prototype, Camera.main.ScreenToWorldPoint(lastPosition)+new Vector3(0,0,10) , Quaternion.identity, fakeShieldCollection);
                ...
                case 2:
                    ...
                    for (int i=0;i<pShieldGroup;i++)
                    {
                        ...
                        lren.SetPosition(i, Camera.main.ScreenToWorldPoint(shieldPointGroup[i]) - Camera.main.transform.position);
                        //Vector2 temp = Camera.main.ScreenToWorldPoint(shieldPointGroup[i]);
                        //lcol.points[i] .Set(temp.x,temp.y);
                        shieldPointGroup2D[i]= Camera.main.ScreenToWorldPoint(shieldPointGroup[i]) - Camera.main.transform.position;
            }
