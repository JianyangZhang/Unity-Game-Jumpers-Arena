           ...
           if (e.gameObject.tag.CompareTo("Player") == 0)
            {
              ...
              //  Onpu.print("Kabe");
				...
				e.gameObject.GetComponent<Player>().location = new Vector3(origin_location.position.x * (-0.99f), origin_location.position.y, 0);

                //e.gameObject.GetComponent<Player>().location += new Vector2(offset * direction, 0);
            }
			if (e.gameObject.tag.CompareTo("MovingFloor") == 0) {
				Debug.Log ("Detected");
				e.gameObject.GetComponent<MovingFloor> ().velocity *= -1;
			}
        }
    }
}